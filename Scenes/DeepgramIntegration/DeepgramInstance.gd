...
func _ready():
	initialize("YOUR_DEEPGRAM_API_KEY")
...
func initialize(api_key):
	...
	if OS.get_name() == "HTML5":
		...
		var err = client.connect_to_url("wss://api.deepgram.com/v1/listen?interim_results=true&encoding=linear16&sample_rate=" + String(mix_rate) + "&channels=1", protocols, false, PoolStringArray())
		if err != OK:
		...
		var err = client.connect_to_url("wss://api.deepgram.com/v1/listen?interim_results=true&encoding=linear16&sample_rate=" + String(mix_rate) + "&channels=1", PoolStringArray(), false, headers)
		if err != OK:
			print("Unable to connect")
			emit_signal("message_received", "unable to connect to deepgram;")
			set_process(false)
